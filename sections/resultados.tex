...
A partir dos dados coletados dos usuários, verificou-se através de validação cruzada\todo{Adicionare referência} que \textit{Random Forest} seria o algoritmo mais apropriado dentre os disponíveis no scikit-learn, com acurácia de aproximadamente \(82\%\) para os dados combinados de todos os usuários, como pode ser visto no gráfico da \autoref{fig:result_accuracy_rfc}.

\todo[inline]{Sugiro adicionar o grafico que apresenta a comparação entre todos os algoritmos e só citar no texto acima.}
...
Ao todo, foram coletadas \(76\) amostras para este cenário. Ao unir ao conjunto de dados de caminhada plana do mesmo usuário, totalizaram-se \(184\) amostras. A acurácia da classificação dos movimentos dos conjuntos de dados combinados foi de apenas \(53\%\)\todo{com qual algoritmo?}.
...
\todo[inline]{Seria bom apresentar o resultado de um número x de execução e descrever se em todas as execuções a simulação funcionou de forma correta.}

\todo[inline]{Outro detalhe a ideia do simulador gerar a animação para a outra perna não foi testada, acho que falamos disso no métod e na introdução.}