...
Uma outra restrição comum que pode ser pertinente a um SE é de tempo. É conhecido como um sistema em tempo real aquele que precisa necessariamente realizar certas operações dentro de um tempo esperado, correndo o risco de não desempenhar seu papel caso isso falhe. Por exemplo, um sistema de controle de velocidade de cruzeiro precisa monitorar a velocidade, aceleração e desaceleração de um carro em tempo real, e, caso ocorra algum atraso, considera-se uma falha no sistema \cite{vahid:2002}.
\textcolor{red}{TODO!}\todo{Descrever as vantagens de sistemas embarcados}
...
Sistemas embarcados\todo{Descrever o porque de estar descrevendo somente estes dois?}\ são muitas vezes usados em situações críticas, em que segurança e confiabilidade são critérios muito importantes\todo{Descrever o que ocorre se um sistema falhar}. O uso de modelos formais para descrever o comportamento do sistema antes que este seja desenvolvido é uma forma de se aproximar da melhor confiabilidade, pois esses modelos podem ser validados automaticamente \cite{edwards:1997}.

\textcolor{red}{TODO}\todo{Adicionar um parágrafo apresentando onde modelos formais são usados e que conhecimento eles geram sobre o sistema}

\subsection{Máquinas de Estado Finitas}

\textcolor{red}{Sugiro renomear está seção para Máquina de estados, então definir e dentro desta seção apresentar as  finitas e não finitas.}
...
\textcolor{red}{Falar sobre as propriedades de análise, ferramentas para análise, e exemplos de quem usa redes de petri.}\todo{TODO}
