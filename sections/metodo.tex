...
O simulador foi projetado para funcionar como uma prótese transtibial, atuando sobre ela para adaptar a rigidez da articulação conforme a situação. Além disso, o simulador conta com a utilização de um classificador de movimentos capaz de prever as ações necessárias nas partes mecânicas via coleta de dados de sensores por meio de um dispositivo vestível na forma de joelheira.

Este dispositivo está equipado com sensores flexíveis \cite{flex:datasheet} nas articulações dos joelhos além de giroscópios e acelerômetros \cite{invensense:imu_mpu}, que serão usados para capturar as ações do usuário. A Figura~\ref{fig:big_picture_prosthesis} ilustra a visão geral do sistema, incluindo o posicionamento dos sensores para o simulador juntamente com uma possível prótese implementada com atuadores reais (Figura~\ref{fig:big_picture_prosthesis}) e a simulação gerada a partir dos dados coletados dos sensores, com a movimentação do pé a partir da classificação destes dados (Figura~\ref{fig:simulation_prototype}).
...
O simulador então trata os dados enviados e realiza a classificação para determinar a ação dos atuadores, conforme fluxograma ilustrado na \autoref{fig:flowchart}, em um modelo virtual da prótese em tempo real.
...
A prótese virtual é apresentada em 3D, visando uma melhor visualização dos componentes propostos e futuro uso de uma impressora 3D com o foco em prototipação da prótese proposta. O uso de impressão 3D tem o intuito de manter o baixo custo do produto.
...
% será decidida um
a técnica de aprendizado de máquina supervisionado para classificação utilizada no simulador é a Floresta Aleatória\todo{Confirmar o nome da técnica}, conforme analisado em testes preliminares feitos e apresentados na Seção~\ref{sec:result_analise}.
