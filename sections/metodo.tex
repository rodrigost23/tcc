...
\todo{Terminar o fluxograma}\begin{figure}[ht]
...
\label{sec:metodo_prototipacao}
\todo[inline,color=lightgray]{Descrever o funcionamento da prótese via algum modelo}

\subsection{Arquitetura do software}
\label{sec:metodo_prot_software}

\begin{figure}[ht]
	\caption{\label{fig:flowchart}Diagrama de sequência que descreve a interação entre os componentes do sistema}
	\begin{center}
	    \includegraphics[width=\textwidth]{resources/sequence_diagram.pdf}
	\end{center}
	\legend{Fonte: Elaborada pelo autor}
\end{figure}\todo{Explicar o diagrama de sequência no texto}

\subsection{Arquitetura do hardware}
\label{sec:metodo_prot_hardware}
...
Além disso, os dados na central serão armazenados para uso posterior de análise, que poderá ser feita separadamente, com o auxílio de um computador (ver \autoref{sec:metodo_diagnostico}).

\section{Previsão de movimentos}
\label{sec:metodo_previsao}
